...
var archiveFilePatterns = tl.getDelimitedInput('archiveFilePatterns', '\n', true);
var destinationFolder = path.normalize(tl.getPathInput('destinationFolder', true, false).trim());
var cleanDestinationFolder = tl.getBoolInput('cleanDestinationFolder', false);
var overwriteExistingFiles = tl.getBoolInput('overwriteExistingFiles', false);
const customPathToSevenZipTool = tl.getInput('pathToSevenZipTool', false);
var repoRoot = tl.getVariable('System.DefaultWorkingDirectory');
tl.debug('repoRoot: ' + repoRoot);
const win = tl.getPlatform() == tl.Platform.Windows;
tl.debug('win: ' + win);
// extractors
var xpTarLocation;
var xpUnzipLocation;
// 7zip
let sevenZipLocation;
let defaultWinSevenZipLocation = path.join(__dirname, '7zip/7z.exe');
...
function findFiles() {
    ...
    for (var i = 0; i < archiveFilePatterns.length; i++) {
        ...
        var normalizedPattern = path.normalize(archiveFilePatterns[i]);
...
function parsePattern(normalizedPattern) {
